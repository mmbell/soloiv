...
/* XPutPixel stub - writes a pixel value into our XImage data buffer */
static inline void XPutPixel(XImage *img, int x, int y, unsigned long pixel) {
  if (img && img->data) {
    int bpp = img->bits_per_pixel / 8;
    if (bpp <= 0) bpp = 4;
    char *dst = (char *)img->data + y * img->bytes_per_line + x * bpp;
    /* Store as little-endian 32-bit */
    dst[0] = (pixel) & 0xff;
    dst[1] = (pixel >> 8) & 0xff;
    dst[2] = (pixel >> 16) & 0xff;
    if (bpp >= 4) dst[3] = (pixel >> 24) & 0xff;
  }
}

/* XAllocColor stub - no-op, color allocation not needed with GTK4 */
#define XAllocColor(display, colormap, xc) (1)
