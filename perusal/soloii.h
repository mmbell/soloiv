...
/* GdkPoint was removed in GTK4; define a simple replacement.
 * Used for widget_origin[] to remember window positions (no-op in GTK4). */
...
/* Pango font description wrapper - replaces GdkFont */
typedef struct {
  PangoFontDescription *desc;
  PangoFontMetrics *metrics;
  gint ascent;   /* in pixels */
  gint descent;  /* in pixels */
} SiiFont;

/* c---------------------------------------------------------------------- */

/* GdkRGBA-based color - replaces GdkColor */
typedef GdkRGBA SiiColor;
...
typedef struct {		/* frame info */
  ...
  GdkPoint widget_origin[MAX_TOP_WINDOWS]; /* window positions (no-op in GTK4) */
  ...
  /* Stored allocation info (replaces direct widget->allocation access) */
  gint alloc_width;
  gint alloc_height;
