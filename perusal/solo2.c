...
/* GTK4 draw function signatures for blow-up */
void sii_blow_up_draw_func (GtkDrawingArea *area, cairo_t *cr,
			    int width, int height, gpointer data);
void sii_blow_up_resize_cb (GtkDrawingArea *area, int width, int height,
			    gpointer data);

/* Event controller callbacks for blow-up */
...
/* Helper: paint RGB data onto a cairo_t using a cairo_image_surface */
static void
sii_cairo_paint_rgb_image (cairo_t *cr, int dest_x, int dest_y,
			   int img_width, int img_height,
			   const guchar *rgb_data, int rowstride)
{
  ...
  surf_data = cairo_image_surface_get_data (surface);
  surf_stride = cairo_image_surface_get_stride (surface);

  cairo_surface_flush (surface);

  for (y = 0; y < img_height; y++) {
    const guchar *src_row = rgb_data + y * rowstride;
    guchar *dst_row = surf_data + y * surf_stride;
    for (x = 0; x < img_width; x++) {
      /* RGB24 in cairo is stored as 0xRRGGBB in native byte order (B, G, R, 0) */
...
/* Helper: draw an arc using cairo.
 * In GTK1, gdk_draw_arc used angles in 1/64th of a degree.
 * Here iangle and iarc_len are in 1/64th of a degree units.
 * Cairo uses radians and measures angles from the positive X axis,
 * counter-clockwise.  GDK also measured counter-clockwise from 3 o'clock,
 * so we can convert directly.
 */
static void
sii_cairo_draw_arc (cairo_t *cr, gboolean filled,
		    int x, int y, int width, int height,
		    int iangle, int iarc_len)
{
...
gboolean sii_reset_frame_sizes ()
{
  ...
  /* In GTK4, we cannot get window origin directly.
   * Use stored allocation or surface-level info.
   * For now, use screen dimensions as upper bound.
   */
  x = 0;
  y = 0;
  ...
  if (height >= dh) {
    nn = dh/sfc->nrows;
    sii_table_widget_height = (nn/4) *4;
  }
  return TRUE;
}
...
{
  GdkRGBA *gcolor;
  gcolor = (GdkRGBA *)g_tree_lookup( colors_tree, (gpointer)"midnightblue" );
}
...
void yes_exit() {
  /* In GTK4, quit via the application */
		       ...
		       struct bnd_point_mgmt *bpm)
{
  ...
  if (sfc->blow_up && sfc0->blow_up_frame) {
    gtk_widget_queue_draw (sfc0->blow_up_frame);
  }
}

/* c------------------------------------------------------------------------ */

/* Draw boundary segments using cairo (called from draw function) */
			     ...
			     struct bnd_point_mgmt *bpm)
{
  ...
  for(jj = 0; jj < num_points-1; jj++, bpm = bpm->last){
    ...
    in1 = x1 >= 0 && x1 < (gint)sfc->width && y1 >= 0 && y1 < (gint)sfc->height;
    in2 = x2 >= 0 && x2 < (gint)sfc->width && y2 >= 0 && y2 < (gint)sfc->height;
...
gboolean sii_blow_up (guint frame_num)
{
  ...
  if (sfc->blow_up) {		/* consecutive hits on this frame. Hide it! */
    if (sfc0->blow_up_frame)
      { gtk_widget_set_visible (sfc0->blow_up_window, FALSE); }
    sfc->blow_up = FALSE;
  ...
  if (sfc0->big_width != sfc->width *2 ||
	...
	gtk_window_destroy (GTK_WINDOW (sfc0->blow_up_window));
      }
      else {
	sfc0->big_image = (SiiImage *)g_malloc0 (sizeof (SiiImage));
      ...
      sfc0->blow_up_window = window = gtk_window_new ();

      sfc0->blow_up_frame = frame = gtk_drawing_area_new ();
      gtk_drawing_area_set_content_width (GTK_DRAWING_AREA (frame),
					  sfc0->big_width);
      gtk_drawing_area_set_content_height (GTK_DRAWING_AREA (frame),
					   sfc0->big_height);
      ...
      gtk_drawing_area_set_draw_func (GTK_DRAWING_AREA (frame),
				      sii_blow_up_draw_func,
				      GUINT_TO_POINTER (frame_num), NULL);

      /* Set up resize callback via GtkDrawingArea::resize signal */
      g_signal_connect (frame, "resize",
			G_CALLBACK (sii_blow_up_resize_cb),
			GUINT_TO_POINTER (frame_num));

      /* Key event controller on the window */
      key_controller = gtk_event_controller_key_new ();
      g_signal_connect (key_controller, "key-pressed",
			G_CALLBACK (sii_blow_up_key_pressed_cb),
			GUINT_TO_POINTER (frame_num));
      gtk_widget_add_controller (window, key_controller);

      /* Click gesture on the drawing area */
      click_gesture = gtk_gesture_click_new ();
      g_signal_connect (click_gesture, "pressed",
			G_CALLBACK (sii_blow_up_mouse_click_cb),
			GUINT_TO_POINTER (frame_num));
      gtk_widget_add_controller (frame, GTK_EVENT_CONTROLLER (click_gesture));
   }

  g_string_printf (gs, "Frame %d", frame_num+1);
  ...
  g_object_set_data (G_OBJECT (frame),
		     "frame_num",
		     GUINT_TO_POINTER (frame_num));
  g_object_set_data (G_OBJECT (sfc0->blow_up_window),
		     "frame_num",
		     GUINT_TO_POINTER (frame_num));

  gtk_widget_set_visible (sfc0->blow_up_window, TRUE);
...
/*
 * sii_really_xfer_images:
 *
 * In GTK4, all drawing must happen in a draw function callback.
 * This function stores the drawing state and queues a redraw.
 * The actual rendering is done in sii_really_xfer_images_cairo.
 */
...
/*
 * sii_really_xfer_images_cairo:
 *
 * This is the core rendering function, called from the draw function
 * with a valid cairo_t. It replaces all the gdk_draw_* calls from the
 * original sii_really_xfer_images.
 */
				   ...
				   GdkRectangle *area, gboolean blow_up)
{
  ...
  font = med_pro_font;
  aa = title_tmplt;
  text_width = sii_font_string_width (font, aa);

  if (text_width > (gint)sfc->width)
    { font = small_pro_font; }
  sfc->font = font;
  sfc->font_height =
  ...
  sii_do_annotation (frame_num, exposed, blow_up, cr);
  ...
  if (sii_center_crosshairs) {
    ...
    y1 = yctr -7;
    y2 = yctr +7;
    cairo_move_to (cr, x1*bup_factor, y1*bup_factor);
    cairo_line_to (cr, x2*bup_factor, y2*bup_factor);
    cairo_stroke (cr);
    y1 = y2 = yctr;
    x1 = xctr -7;
    x2 = xctr +7;
  ...
  for (; !time_series && rng > 0 && rng < max;
	...
	*/
	 ...
	 { continue; }
	  ...
	  sii_cairo_draw_arc (cr, FALSE, x, y, width, height,
			      iangle, iarc_len);
       }
    }
    if (angle < 0)
  ...
  stp = &sfc->tbl_parms;
  width = gtk_widget_get_width (frame);
  height = gtk_widget_get_height (frame);


  /* draw a two pixel width border */
  sii_cairo_set_color (cr, frame_border_colors[wwptr->lock_num]);
...
# ifdef config_debug
     sprintf (str+strlen(str), " data:%p", (void *)sfc->image->data);
     /*
     g_message (str);
      */
