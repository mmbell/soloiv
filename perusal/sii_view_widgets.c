...
sii_set_view_info (guint frame_num)
{
  ...
  jj = VIEW_ZOOM;
  aa = gtk_editable_get_text( GTK_EDITABLE (vd->data_widget[jj]));
...
void sii_update_view_widget (guint frame_num)
{
  ...
  gtk_check_button_set_active(GTK_CHECK_BUTTON(check_item), TRUE);
   ...
   gtk_editable_set_text (GTK_EDITABLE (vd->data_widget[jj]), aa);
...
gint sii_radio_inventory ( GtkWidget *rmi, gint *item_count )
{
   ...
   /* GTK4: GtkCheckButton with set_group replaces GtkRadioMenuItem.
    * We can't easily enumerate the group, so just check if this item is active.
    */
   (void)group;
   *item_count = 1;
   if (GTK_IS_CHECK_BUTTON(rmi) && gtk_check_button_get_active(GTK_CHECK_BUTTON(rmi)))
     { kk = 0; }
   return kk;
...
# endif
  ...
  case VIEW_TS_MODE:
    ...
    vd->toggle[task] = gtk_check_button_get_active(GTK_CHECK_BUTTON(rmi));
# ifdef obsolete
    g_message ( "View Task: %d  State: %d  Active: %d"
		, task, vd->toggle[task], active );
# endif
...
# endif
  ...
  case VIEW_LMRK_REF_FRAME:
				/* one value */
    aa = gtk_editable_get_text( GTK_EDITABLE (vd->data_widget[task]));
...
void sii_view_entry_paste_cb ( GtkWidget *w, gpointer   data )
{
   ...
   gtk_editable_set_text (GTK_EDITABLE (vdata->data_widget[wid]), "");
...
void show_view_widget (GtkWidget *text, gpointer data )
{
  ...
  if( !widget )
    { sii_view_widget( frame_num ); }
     ...
     x = ptp->x; y = ptp->y;
     /* GTK4: window positioning is managed by the compositor */
...
void sii_view_widget( guint frame_num )
{
  ...
  g_free( bb );
  gtk_widget_set_margin_start (window, 0);
  gtk_widget_set_margin_end (window, 0);
  gtk_widget_set_margin_top (window, 0);
  gtk_widget_set_margin_bottom (window, 0);
  ...
  /* GTK4: spacing set in gtk_box_new above */
...
# endif
  ...
  /* GTK4: iterate children using gtk_widget_observe_children() */
  {
    GtkWidget *child;
    for (child = gtk_widget_get_first_child (vbox);
         child != NULL;
   ...
   *menubar = mbar = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
   ...
   vdata->data_widget[widget_id] = menuitem =
     sii_submenu_item ( "Close", submenu, widget_id
		       , G_CALLBACK (sii_view_menu_cb), frame_num );
...
# endif
   ...
   vdata->data_widget[widget_id] = menuitem =
     sii_toggle_submenu_item ( "Az-Rng Overlay", submenu, widget_id
			       , G_CALLBACK (sii_view_menu_cb), frame_num, ++radio_num
			       , &radio_group);
   ...
   vdata->data_widget[widget_id] = menuitem =
     sii_toggle_submenu_item ( "Az-Rng Labels", submenu, widget_id
			       , G_CALLBACK (sii_view_menu_cb), frame_num, 0
			       , &radio_group);
