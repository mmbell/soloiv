...
void sii_param_add_ctbl(const char **at, int nn);  /* sii_param_widgets.c */

gboolean Xcolors_flag = no;

/* c---------------------------------------------------------------------- */
GdkRGBA *normalized_RGB_to_gdk( gfloat f_red, gfloat f_green, gfloat f_blue );
...
GdkRGBA *sii_set_gtk_color(  gchar *color_name
			       , gfloat f_red, gfloat f_green, gfloat f_blue );

GdkRGBA *tone_down_gdk_color( gchar *name, gfloat attenuation );
...
GdkRGBA *normalized_RGB_to_gdk( gfloat f_red, gfloat f_green, gfloat f_blue )
{
  ...
  c = sii_color_new( f_red, f_green, f_blue );
...
GdkRGBA *tone_down_gdk_color( gchar * name, gfloat atten )
{
				/* 0 < value < 1.0 */
  GdkRGBA *old, *c;
  ...
  old = (GdkRGBA *)g_hash_table_lookup( colors_hash, (gpointer)name );
			      ...
			      , gfloat f_red, gfloat f_green, gfloat f_blue )
{
  ...
  GdkRGBA *gcolor = normalized_RGB_to_gdk( f_red, f_green, f_blue );
  GdkRGBA *gcolor2;

  new_name = (gchar *)g_malloc0( strlen( color_name ) +1 );
  strcpy( new_name, color_name );

  gcolor2 = (GdkRGBA *)g_tree_lookup( colors_tree, (gpointer)new_name );
  if( gcolor2 ) {
    return (gcolor);
  }
  ...
  gcolor2 = (GdkRGBA *)g_hash_table_lookup( colors_hash, (gpointer)new_name );
				/*
				 */
  return (gcolor);
}
...
void sii_set_default_colors ()
{
  ...
  g_hash_table_foreach ( colors_hash, sii_print_hash_str, 0 );
				 */
  ii = 0;
  frame_border_colors[ii++] = (GdkRGBA *)g_hash_table_lookup
