...
void sii_edit_reset_times (guint frame_num)
{
   ...
   gtk_editable_set_text (GTK_EDITABLE (edd->data_widget[EDIT_START_TIME]), cc );

   cc = sii_string_from_dtime (edd->stop_time);
   g_string_assign (edd->orig_txt[EDIT_STOP_TIME], cc);
   gtk_editable_set_text (GTK_EDITABLE (edd->data_widget[EDIT_STOP_TIME]), cc );
}
...
void sii_edit_menu_cb ( GtkWidget *w, gpointer data )
{
   ...
   switch (task) {
    ...
    case EDIT_DO_IT:
      /* Get text from GtkTextView buffers */
      buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(edd->data_widget[EDIT_FER_TEXT]));
      {
        GtkTextIter start_iter, end_iter;
        gtk_text_buffer_get_start_iter(buffer, &start_iter);
        gtk_text_buffer_get_end_iter(buffer, &end_iter);
        seds->fer_lines = gtk_text_buffer_get_text(buffer, &start_iter, &end_iter, FALSE);
      }
      buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(edd->data_widget[EDIT_OTO_TEXT]));
      {
        GtkTextIter start_iter, end_iter;
        gtk_text_buffer_get_start_iter(buffer, &start_iter);
        gtk_text_buffer_get_end_iter(buffer, &end_iter);
        seds->oto_lines = gtk_text_buffer_get_text(buffer, &start_iter, &end_iter, FALSE);
      }
    ...
    case EDIT_BND_OUT:
      ...
      if ((active = gtk_check_button_get_active(GTK_CHECK_BUTTON(check_item))))
    ...
    case EDIT_BND_DRAW:
      edd->toggle[task] = gtk_check_button_get_active(GTK_CHECK_BUTTON(rmi));
      break;
    ...
    case EDIT_CMD_DIR:
      cc = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_CMD_DIR]));
      if (!strlen (cc)) {
    ...
    case EDIT_CMD_COMM:
	 ...
	 cc = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_CMD_DIR]));
	 bb = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_CMD_COMM]));
      }
      else {
	 cc = sii_return_swpfi_dir (frame_num);
	 bb = "_no_comment";
    ...
    case EDIT_LLB_DIR:		/* lat/lon boundary (LLB) */
      cc = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_LLB_DIR]));
      if (!strlen (cc)) {
	cc = sii_return_swpfi_dir (frame_num);
      }
    ...
    case EDIT_BND_DIR:
      cc = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_BND_DIR]));
      if (!strlen (cc)) {
	cc = sii_return_swpfi_dir (frame_num);
      }
    ...
    case EDIT_BND_COMM:
	 ...
	 cc = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_BND_DIR]));
	 bb = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_BND_COMM]));
      }
      else {
	 cc = sii_return_swpfi_dir (frame_num);
	 bb = "_no_comment";
    ...
    case EDIT_LAST_SWP:
	 ...
	 cc = "-1";
      ...
      gtk_editable_set_text (GTK_EDITABLE (edd->data_widget[wid]), cc );
      break;

    case EDIT_START_TIME:
    case EDIT_STOP_TIME:
      aa = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[wid]));
      strcpy (str, aa);
      g_strstrip (str);
      if (strcmp (str, "0") == 0) {
	 dtime = DAY_ZERO; cc = "0";
...
int se_edit_start_stop (int frame_num, struct swp_file_input_control *sfic)
{
  ...
  aa = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_START_TIME]));
  strcpy (str, aa);
  g_strstrip (str);
  if (strcmp (str, "0") == 0) {
    dtime = DAY_ZERO;
  ...
  bb = gtk_editable_get_text (GTK_EDITABLE (edd->data_widget[EDIT_STOP_TIME]));
  strcpy (str, bb);
  g_strstrip (str);
  if (strcmp (str, "-1") == 0) {
    dtime = ETERNITY;
			       ...
			       , gpointer data )
{
  ...
  if (wid == EDIT_CMD_FILES) {
	...
	g_string_append (gs, "\n");
     ...
     if (gs->len) {
	buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(edd->data_widget[EDIT_OTO_TEXT]));
	gtk_text_buffer_insert_at_cursor(buffer, gs->str, -1);
     }
     g_string_truncate (gs, 0);
...
void edit_files_widget (guint frame_num )
{
  ...
  g_free( bb );
  gtk_widget_set_margin_start (GTK_WIDGET (window), 2);
  gtk_widget_set_margin_end (GTK_WIDGET (window), 2);
  gtk_widget_set_margin_top (GTK_WIDGET (window), 2);
  gtk_widget_set_margin_bottom (GTK_WIDGET (window), 2);
  ...
  gtk_widget_set_margin_start (GTK_WIDGET (hbox), 4);
  gtk_widget_set_margin_end (GTK_WIDGET (hbox), 4);
  ...
  gtk_editable_set_text (GTK_EDITABLE (entry), edd->orig_txt[wid]->str );
  ...
  gtk_grid_attach (GTK_GRID (grid), scrolled_window, 0, row, 6, 1);
  row++;
...
void main_edit_widget( guint frame_num )
{
  ...
  width = sii_font_string_width( small_pro_font, title_tmplt );
  ...
  if (!cmds_one_time_only) {
	...
	width = sii_font_string_width( edit_font, str );
	if( width > max_cmd_width )
	  { max_cmd_width = width; }
     }
     max_cmd_width += sii_font_string_width( edit_font, "WWW" );
  }
  ...
  gtk_box_append (GTK_BOX(vbox), edit_menubar);
  ...
  gtk_widget_set_margin_start (GTK_WIDGET (vbox1), 2);
  gtk_widget_set_margin_end (GTK_WIDGET (vbox1), 2);
  gtk_widget_set_margin_top (GTK_WIDGET (vbox1), 2);
  gtk_widget_set_margin_bottom (GTK_WIDGET (vbox1), 2);
  ...
  gtk_box_append (GTK_BOX (vbox1), grid);
  row = -1;
...
# endif
  ...
  gtk_widget_set_size_request (button, 0, 32);
  gtk_widget_set_hexpand (button, TRUE);
  gtk_widget_set_vexpand (button, TRUE);
  row++;
  gtk_grid_attach (GTK_GRID (grid), button, 0, row, 1, 1);
  ...
  gtk_widget_set_size_request (label, 0, 32);
  gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_CENTER );
  gtk_widget_set_hexpand (label, TRUE);
  gtk_box_append (GTK_BOX (hbox1), label);

  button = gtk_button_new_with_label ("Clear");
  gtk_widget_set_size_request (button, 0, 28);
  gtk_widget_set_hexpand (button, TRUE);
  ...
  edata->data_widget[EDIT_FER_TEXT] = text = gtk_text_view_new ();

  scrolled_window = gtk_scrolled_window_new ();
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_window),
                                  GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_widget_set_size_request (scrolled_window, max_cmd_width, 112);
  gtk_scrolled_window_set_child (GTK_SCROLLED_WINDOW (scrolled_window), text);
  gtk_widget_set_hexpand (scrolled_window, TRUE);
  gtk_widget_set_vexpand (scrolled_window, TRUE);
  gtk_box_append (GTK_BOX (vbox1), scrolled_window);
  ...
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_window),
                                  GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_widget_set_size_request (scrolled_window, max_cmd_width, 168);
  gtk_scrolled_window_set_child (GTK_SCROLLED_WINDOW (scrolled_window), text);
  gtk_widget_set_hexpand (scrolled_window, TRUE);
  gtk_widget_set_vexpand (scrolled_window, TRUE);
  gtk_box_append (GTK_BOX (vbox1), scrolled_window);
 ...
 /* one time only commands */
  ...
  gtk_widget_set_margin_start (GTK_WIDGET (vbox3), 2);
  gtk_widget_set_margin_end (GTK_WIDGET (vbox3), 2);
  gtk_widget_set_margin_top (GTK_WIDGET (vbox3), 2);
  gtk_widget_set_margin_bottom (GTK_WIDGET (vbox3), 2);
  gtk_widget_set_hexpand (vbox3, TRUE);
  gtk_box_append (GTK_BOX (hbox), vbox3);
  ...
  edata->data_widget[EDIT_OTO_TEXT] = text = gtk_text_view_new ();
  ...
  gtk_widget_set_hexpand (scrolled_window, TRUE);
  gtk_widget_set_vexpand (scrolled_window, TRUE);
  gtk_box_append (GTK_BOX (vbox3), scrolled_window);
  gtk_text_view_set_editable (GTK_TEXT_VIEW (text), TRUE);


  label = gtk_label_new ( "List of All One Time Only Commands" );
  gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_CENTER );
  gtk_box_append (GTK_BOX (vbox3), label);
  ...
  /* GTK4: iterate children manually instead of gtk_container_foreach */
  {
   ...
   *menubar = mbar = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
   ...
   edata->data_widget[widget_id] = menuitem =
     sii_submenu_item ( "Close", submenu, widget_id
		       , G_CALLBACK(sii_edit_menu_cb), frame_num );
...
# endif
   ...
   edata->data_widget[widget_id] = menuitem =
     sii_toggle_submenu_item ( "Auto Replot", submenu, widget_id
		       , G_CALLBACK(sii_edit_menu_cb), frame_num, 0, &radio_group );
   ...
   edata->data_widget[widget_id] = menuitem =
     sii_toggle_submenu_item ( "Edit Inside", submenu, widget_id
		       , G_CALLBACK(sii_edit_menu_cb), frame_num, ++radio_num
			       , &radio_group);
...
gboolean set_edit_help_items( gpointer a_str, gpointer words )
{
  ...
  /* GTK4: GtkEventBox removed; widgets receive events directly */
